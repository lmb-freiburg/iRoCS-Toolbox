...
**    $RCSfile$
**   $Revision: 3836 $$Name$
**       $Date: 2010-06-30 22:31:41 +0200 (Mi, 30 Jun 2010) $
...
**
**  $Log$
**  Revision 1.2  2008/01/30 14:28:00  schlacht
**  -- LMBUNIT_ASSERT_EXCEPTION now catches std::exception`s too
**
**  Revision 1.1  2007/11/27 00:20:01  schlacht
**  unit test template header
**
**  Revision 1.2  2007/10/18 14:42:15  schlacht
**  -- added macro for testing on exceptions
**
**  Revision 1.1  2007/10/17 11:25:03  schlacht
**  -- header files for unit tests
**
...
static bool _CAUGHT_LMBUNIT_ERROR = false;

#define LMBUNIT_ASSERT_EXCEPTION( expected_error)                             \
catch(expected_error e)                                                \
{\
  _CAUGHT_LMBUNIT_ERROR = true;\
  std::cout << (#expected_error) << "\n"; \
}\
catch(std::exception& e) \
  {\
  LMBUNIT_WRITE_FAILURE("expected error `" << (#expected_error) <<\
                        "` was not catched. Catched std::exception.\n"\
                        "Error message:\n" << e.what());    \
  } \
catch(...){ \
  _CAUGHT_LMBUNIT_ERROR = true;\
  LMBUNIT_WRITE_FAILURE("expected error `" << (#expected_error) <<\
                        "` was not catched. Catched different error.");    \
}\
if(!_CAUGHT_LMBUNIT_ERROR){\
LMBUNIT_WRITE_FAILURE("expected error was`" << (#expected_error) <<\
                        "` but didn`t catch anything.");\
_CAUGHT_LMBUNIT_ERROR = false;\
}
