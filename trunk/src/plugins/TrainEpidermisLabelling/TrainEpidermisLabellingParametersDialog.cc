          ...
          p_mainWidget(mainWidget)
{
  ...
  p_randomSamplesGroup->setChecked(
      settings.value(
          "iRoCSPipeline/TrainEpidermisLabelling/generateRandomSamples",
          false).toBool());
  p_nInRootSamplesControlElement->setValue(
      settings.value("iRoCSPipeline/TrainEpidermisLabelling/nInRootSamples",
                     0).toInt());
  p_nOutRootSamplesControlElement->setValue(
      settings.value("iRoCSPipeline/TrainEpidermisLabelling/nOutRootSamples",
                     0).toInt());
  p_modelFileNameControlElement->setValue(
      settings.value("iRoCSPipeline/EpidermisLabellingModel",
                     "detector_model.h5").toString().toStdString());
  p_sdNormalizationControl->setValue(
      static_cast<iRoCS::Features::NormalizationType>(
          settings.value(
              "iRoCSPipeline/TrainEpidermisLabelling/sdNormalization",
              3).toInt()));
  p_houghNormalizationControl->setValue(
      static_cast<iRoCS::Features::NormalizationType>(
          settings.value(
              "iRoCSPipeline/TrainEpidermisLabelling/houghNormalization",
              1).toInt()));
  p_costControlElement->setValue(
      settings.value("iRoCSPipeline/TrainEpidermisLabelling/cost",
                     10.0).toDouble());
  p_gammaControlElement->setValue(
      settings.value("iRoCSPipeline/TrainEpidermisLabelling/gamma",
                     0.01).toDouble());  
...
TrainEpidermisLabellingParametersDialog::trainFiles() const
{
  std::vector<TrainfileParameters*> res;
  for (int i = 0; i < p_trainFileList->topLevelItemCount(); ++i)
      res.push_back(
          static_cast<TrainEpidermisLabellingTrainfileParametersItem*>(
              p_trainFileList->topLevelItem(i)));
  return res;
}

void TrainEpidermisLabellingParametersDialog::setFeatureGroup(
    std::string const &group)
{
  p_featureGroupControlElement->setValue(group);
}

std::string TrainEpidermisLabellingParametersDialog::featureGroup() const
{
  return p_featureGroupControlElement->value();
}

void TrainEpidermisLabellingParametersDialog::setGenerateRandomSamples(
    bool generate)
{
  p_randomSamplesGroup->setChecked(generate);
}

bool TrainEpidermisLabellingParametersDialog::generateRandomSamples() const
{
  return p_randomSamplesGroup->isChecked();
}

void TrainEpidermisLabellingParametersDialog::setNInRootSamples(int samples)
{
  p_nInRootSamplesControlElement->setValue(samples);
}

int TrainEpidermisLabellingParametersDialog::nInRootSamples() const
{
  return p_nInRootSamplesControlElement->value();
}

void TrainEpidermisLabellingParametersDialog::setNOutRootSamples(int samples)
{
  p_nOutRootSamplesControlElement->setValue(samples);
}

int TrainEpidermisLabellingParametersDialog::nOutRootSamples() const
{
  return p_nOutRootSamplesControlElement->value();
}

void TrainEpidermisLabellingParametersDialog::setModelFileName(
    std::string const &name)
{
  p_modelFileNameControlElement->setValue(name);
}

std::string TrainEpidermisLabellingParametersDialog::modelFileName() const
{
  return p_modelFileNameControlElement->value();
}

void TrainEpidermisLabellingParametersDialog::setSdFeatureNormalization(
...
TrainEpidermisLabellingParametersDialog::sdFeatureNormalization() const
{
  return static_cast<iRoCS::Features::NormalizationType>(
      p_sdNormalizationControl->value());
}

void TrainEpidermisLabellingParametersDialog::setHoughFeatureNormalization(
    iRoCS::Features::NormalizationType normType)
{
...
iRoCS::Features::NormalizationType
TrainEpidermisLabellingParametersDialog::houghFeatureNormalization() const
{
  return static_cast<iRoCS::Features::NormalizationType>(
      p_houghNormalizationControl->value());
}

void TrainEpidermisLabellingParametersDialog::setCost(double cost)
{
  p_costControlElement->setValue(cost);
}

double TrainEpidermisLabellingParametersDialog::cost() const
{
  return p_costControlElement->value();
}

void TrainEpidermisLabellingParametersDialog::setGamma(double gamma)
{
  p_gammaControlElement->setValue(gamma);
}

double TrainEpidermisLabellingParametersDialog::gamma() const
{
  return p_gammaControlElement->value();
}

void TrainEpidermisLabellingParametersDialog::checkAndAccept()
{
  // Check input files
  ...
  settings.setValue(
      "iRoCSPipeline/TrainEpidermisLabelling/nInRootSamples",
      nInRootSamples());
  settings.setValue(
      "iRoCSPipeline/TrainEpidermisLabelling/nOutRootSamples",
      nOutRootSamples());
  settings.setValue("iRoCSPipeline/EpidermisLabellingModel",
                    modelFileName().c_str());
  settings.setValue(
      "iRoCSPipeline/TrainEpidermisLabelling/sdNormalization",
  ...
  settings.setValue("iRoCSPipeline/TrainEpidermisLabelling/cost", cost());
  settings.setValue("iRoCSPipeline/TrainEpidermisLabelling/gamma", gamma());  

  accept();
}

void TrainEpidermisLabellingParametersDialog::addTrainFile()
{
  QSettings settings;
  ...
  TrainEpidermisLabellingTrainfileParametersItem *trainFile =
      new TrainEpidermisLabellingTrainfileParametersItem(
