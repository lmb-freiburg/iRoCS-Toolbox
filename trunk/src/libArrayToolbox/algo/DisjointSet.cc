...
// of course destroy the data structure, but we just need the indexes of roots after processing
void
DisjointSets::ReNum()
{
  int counter = 0;
  ...
  {
      if (FindSet(m_nodes[k]->index) == k)
      {
          m_nodes[k]->index = counter++;
      }
  }
  assert(counter ==  m_numSets);
}
