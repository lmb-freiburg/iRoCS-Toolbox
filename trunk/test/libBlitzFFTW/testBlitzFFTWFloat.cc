...
{
  BlitzFFTW<float>* fftProc = BlitzFFTW<float>::instance();
  LMBUNIT_ASSERT_EQUAL(BlitzFFTW<float>::instance(), fftProc);
}
...
static void testGetPaddedShape() 
{
  ...
  LMBUNIT_ASSERT_EQUAL(
      BlitzFFTW<float>::instance()->getPaddedShape(shape1D)(0), 8);
  blitz::TinyVector<int,2> shape2D(1, 25);
  LMBUNIT_ASSERT_EQUAL(
      BlitzFFTW<float>::instance()->getPaddedShape(shape2D)(0), 2);
  LMBUNIT_ASSERT_EQUAL(
      BlitzFFTW<float>::instance()->getPaddedShape(shape2D)(1), 30);
  blitz::TinyVector<int,3> shape3D(15, 27, 13);
  LMBUNIT_ASSERT_EQUAL(
      BlitzFFTW<float>::instance()->getPaddedShape(shape3D)(0), 16);
  LMBUNIT_ASSERT_EQUAL(
      BlitzFFTW<float>::instance()->getPaddedShape(shape3D)(1), 30);
  LMBUNIT_ASSERT_EQUAL(
      BlitzFFTW<float>::instance()->getPaddedShape(shape3D)(2), 16);
}

static void testPadUnpad() 
{
  blitz::Array<float,1> data1D(7);

  for (int i = 0; i < 7; ++i)
      data1D(i) = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);

  blitz::Array<float,1> padded1D;
  blitz::TinyVector<int,1> lb1D;
  blitz::TinyVector<int,1> ub1D;

  BlitzFFTW<float>::instance()->pad(data1D, padded1D, lb1D, ub1D);
  ...
  blitz::Array<float,1> unpadded1D;
  BlitzFFTW<float>::instance()->unpad(padded1D, unpadded1D, lb1D, ub1D);
  
  LMBUNIT_ASSERT_EQUAL(unpadded1D.extent(0), 7);
  for (int i = 0; i < 7; ++i)
      LMBUNIT_ASSERT_EQUAL(data1D(i), unpadded1D(i));

  blitz::Array<float,2> data2D(1, 25);

  for (int i = 0; i < 25; ++i) 
      data2D(0, i) = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);

  blitz::Array<float,2> padded2D;
  blitz::TinyVector<ptrdiff_t,2> lb2D;
  blitz::TinyVector<ptrdiff_t,2> ub2D;
  blitz::TinyVector<ptrdiff_t,2> minShape2(data2D.shape());

  BlitzFFTW<float>::instance()->pad(data2D, padded2D, lb2D, ub2D,
                                     minShape2, BlitzFFTW<float>::VALUE, 42.0);
  ...
  blitz::Array<float,2> unpadded2D;
  BlitzFFTW<float>::instance()->unpad(padded2D, unpadded2D, lb2D, ub2D);
  ...
  blitz::Array<float,3> data3D(15, 27, 13);

  for (blitz::Array<float,3>::iterator it = data3D.begin();
       it != data3D.end(); ++it) 
  {
    *it = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
  }
  
  blitz::Array<float,3> padded3D;
  blitz::TinyVector<ptrdiff_t,3> lb3D;
  blitz::TinyVector<ptrdiff_t,3> ub3D;

  blitz::TinyVector<ptrdiff_t,3> minShape = (3 * data3D.shape()) / 2;

  BlitzFFTW<float>::instance()->pad(data3D, padded3D, lb3D, ub3D,
                                    minShape,
                                    BlitzFFTW<float>::REPEATBORDER);
  ...
  blitz::Array<float,3> unpadded3D;
  BlitzFFTW<float>::instance()->unpad(padded3D, unpadded3D, lb3D, ub3D);
...
static void testFFT1DFloatWithDataPreserval() 
{
  int baseSize = 12;

  BlitzFFTW<float>* fftProc = BlitzFFTW<float>::instance();
  
  blitz::Array<float,1> data(baseSize);
  blitz::Array<float,1> dataCopy(baseSize);
  blitz::Array<std::complex<float>,1> fft(baseSize / 2 + 1);
  blitz::Array<std::complex<float>,1> fftCopy(baseSize / 2 + 1);
  ...
  for (int i = 0; i < baseSize / 2 + 1; ++i)
      fft(i) = std::complex<float>(static_cast<float>(rand()) / static_cast<float>(RAND_MAX),
...
static void testFFT1DFloatWithoutDataPreserval() 
{
  int baseSize = 12;
...
static void testFFT2DFloatWithDataPreserval() 
{
  int baseSize = 12;

  BlitzFFTW<float>* fftProc = BlitzFFTW<float>::instance();
  
  blitz::Array<float,2> data(baseSize, baseSize);
  blitz::Array<float,2> dataCopy(baseSize, baseSize);
  blitz::Array<std::complex<float>,2> fft(baseSize, baseSize / 2 + 1);
  blitz::Array<std::complex<float>,2> fftCopy(baseSize, baseSize / 2 + 1);

  for (blitz::Array<float,2>::iterator it = data.begin();
       it != data.end(); ++it)
      *it = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
  dataCopy = data;

  for (blitz::Array<std::complex<float>,2>::iterator it = fft.begin();
       it != fft.end(); ++it)
      *it = std::complex<float>(static_cast<float>(rand()) / static_cast<float>(RAND_MAX),
                                static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
  fftCopy = fft;
...
static void testFFT2DFloatWithoutDataPreserval() 
{
  int baseSize = 12;
...
{
  blitz::Array<float,3> data(10, 10, 10);
  data = 0;
  data(blitz::TinyVector<ptrdiff_t,3>(data.shape() / 2)) = 1;
  BlitzFFTW<float>::instance()->unShuffle(data, data);
...
int main() 
{
  ...
  LMBUNIT_RUN_TEST(testFFT1DFloatWithDataPreserval());
  LMBUNIT_RUN_TEST(testFFT1DFloatWithoutDataPreserval());
  LMBUNIT_RUN_TEST(testFFT2DFloatWithDataPreserval());
  LMBUNIT_RUN_TEST(testFFT2DFloatWithoutDataPreserval());
