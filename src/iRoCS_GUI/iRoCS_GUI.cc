...
int main(int argc, char**argv) {
  ...
  try {
    ...
    iRoCS::iRoCSMainWindow *w = NULL;
    if (inFileName.given())
      w = new iRoCS::iRoCSMainWindow(
        mem, inFileName.value().c_str(), forceImport.given());
    else w = new iRoCS::iRoCSMainWindow(mem);
  ...
  catch (CmdLineVersionError &) {
    std::cout << PACKAGE_STRING << std::endl;
    exit(0);
  }
  catch (CmdLineLicenseError &) {
  ...
  catch (CmdLineUsageHTMLError &) {
    cmd.usageHTML(std::cout);
    exit(0);
  }
  catch (CmdLineUsageXMLError &) {
    ...
    std::cerr << "An exception occurred during execution of the iRoCS toolbox. "
      << "If the error is not memory related, report it!" << std::endl
      << "  Error Message: " << e.what() << std::endl;
    exit(-1);
  }
  catch (...) {
    std::cerr << "Unknown error occurred. Sorry no further information "
      << "available" << std::endl;
    exit(-1);
  }
}
