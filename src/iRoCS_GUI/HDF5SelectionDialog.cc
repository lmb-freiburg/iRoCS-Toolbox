...
namespace iRoCS {
  ...
  HDF5TreeWidget::HDF5TreeWidget(
    iRoCS::ProgressReporter *pr, QWidget *parent)
    : QTreeWidget(parent), _fileName(""), p_progress(pr), _itemTypes(),
  ...
  HDF5TreeWidget::HDF5TreeWidget(
    ...
    QWidget *parent)
    : QTreeWidget(parent), _fileName(fileName), p_progress(pr),
    _itemTypes(), _itemChannelTypes(), _valid(false), _errorString("") {
    _generateTree();
  }
  ...
  void HDF5TreeWidget::setSelectable(Channel::Types types, bool enable) {
    for (std::map<QTreeWidgetItem*, Channel::Type>::iterator it =
  ...
  std::string HDF5TreeWidget::absoluteItemPath(
  ...
  void HDF5TreeWidget::_createGroupItem(
    ...
    if (inFile.existsDataset(groupName + "/position_um")) {
      ...
      _itemChannelTypes[item] = Channel::Annotation;
    }
    else {
      try {
        ...
        _itemTypes[item] = Annotation;
        _itemChannelTypes[item] = Channel::SplineCurve;
      }
      catch (BlitzH5Error &) {
      }
      try {
        ...
        _itemTypes[item] = Annotation;
        _itemChannelTypes[item] = Channel::IRoCS;
      }
      catch (BlitzH5Error &) {
      }
      try {
        ...
        _itemTypes[item] = Annotation;
        _itemChannelTypes[item] = Channel::IRoCSSCT;
      }
      catch (BlitzH5Error &) {
      }
      // try
  ...
  void HDF5TreeWidget::_createDatasetItem(
    ...
    if (nDims <= 1) item->setText(3, tr("Scalar"));
      ...
      if (extents.back() == 3) {
        ...
        // treated as RGB data. For other dimensions check the
        // dim_interpretation attribute if it exists
        bool isRGB = false;
        if (extents.size() == 4) isRGB = true;
          ...
          try {
            ...
            inFile.readAttribute(
              dimInterpretation, "dim_interpretation", dsName);
        ...
        if (isRGB) {
          ...
          _itemChannelTypes[item] = Channel::RGB;
        }
      }
  ...
  void HDF5TreeWidget::_addH5Objects(
    ...
    int pMin = (p_progress != NULL) ? p_progress->taskProgressMin() : 0;
    int pMax = (p_progress != NULL) ? p_progress->taskProgressMax() : 100;
    if (p_progress != NULL) p_progress->updateProgress(pMin);

    std::string groupName = absoluteItemPath(group);
    std::vector<std::string> objects(inFile.getObjects(groupName));

    float pScale = (p_progress != NULL) ?
      static_cast<float>(pMax - pMin) / static_cast<float>(objects.size()) :
      100.0f;
    ...
    for (std::vector<std::string>::iterator it = objects.begin();
      it != objects.end(); ++it, ++i) {
      if (p_progress != NULL) {
        p_progress->setTaskProgressMin(static_cast<int>(pMin + i * pScale));
        p_progress->setTaskProgressMax(
          static_cast<int>(pMin + (i + 1) * pScale));
        p_progress->updateProgress(static_cast<int>(pMin + i * pScale));
      }
    ...
    if (p_progress != NULL) {
      p_progress->setTaskProgressMin(pMin);
      p_progress->setTaskProgressMax(pMax);
      p_progress->updateProgress(pMax);
    }
  }

  void HDF5TreeWidget::_generateTree() {
    if (p_progress != NULL)
      p_progress->updateProgressMessage("Analyzing HDF5 file");
    clear();
    setColumnCount(5);
  ...
  *-----------------------------------------------------------------------*/
  ...
  std::vector<std::string> HDF5SelectionDialog::selectObjects(
    ...
    Channel::Types types, iRoCS::ProgressReporter *pr) {
    HDF5SelectionDialog* dlg =
      new HDF5SelectionDialog(filename, model, parent, types, pr);
  ...
  HDF5SelectionDialog::HDF5SelectionDialog(
    ...
    p_h5TreeWidget = new HDF5TreeWidget(filename.toStdString(), pr);
