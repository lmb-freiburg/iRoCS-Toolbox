...
IRoCSSCTChannelSpecsOrthoViewRenderer::IRoCSSCTChannelSpecsOrthoViewRenderer(
...
void IRoCSSCTChannelSpecsOrthoViewRenderer::userInteractionEvent(
    const UserInteractionEvent&)
{}

void IRoCSSCTChannelSpecsOrthoViewRenderer::render(QPainter* painter) const
{
  if (painter == NULL)
  {
    std::cerr << "IRoCSSCTChannelSpecsOrthoViewRenderer::render(): "
  ...
  blitz::TinyVector<double,3> posUm(channel->p_sct->getAxisPosition(start));
  blitz::TinyVector<double,3> posPx(view->um2Px(posUm));
  mainAxisPath.moveTo(QPointF(posPx(dims(1)), posPx(dims(0))));
  for (double u = start + step; u <= end; u += step)
  {
    posUm = channel->p_sct->getAxisPosition(u);
  ...
  atb::SurfaceGeometry const &surf = channel->p_sct->surface();

  for (size_t triangle = 0; triangle < surf.indices().size() / 3; ++triangle)
  {
    blitz::TinyVector<blitz::TinyVector<double,3>,3> tri(
        surf.vertices()[surf.indices()[3 * triangle]],
        surf.vertices()[surf.indices()[3 * triangle + 1]],
        surf.vertices()[surf.indices()[3 * triangle + 2]]);
  ...
  /*-----------------------------------------------------------------------
   *  Extend axis by linear extrapolation
   *-----------------------------------------------------------------------*/  
  ...
  blitz::TinyVector<double,3> pos(channel->p_sct->getAxisPosition(start));
  blitz::TinyVector<double,3> dir(-channel->p_sct->getAxisDirection(start));
  ...
  for (int d = 0; d < 6; ++d)
  {
    if (lambda(d) > 0)
    {
                     ...
                     posUm <= model->upperBoundUm()))
      {
        ...
        posPx = view->um2Px(channel->p_sct->getAxisPosition(start));
  ...
  pos = channel->p_sct->getAxisPosition(end);
  dir = channel->p_sct->getAxisDirection(end);
  ...
  for (int d = 0; d < 6; ++d)
  {
    if (lambda(d) > 0)
    {
      ...
      if (blitz::all(posUm >= model->lowerBoundUm() &&
                     posUm <= model->upperBoundUm()))
      {
        posPx = view->um2Px(channel->p_sct->getAxisPosition(end));
        path.moveTo(QPointF(posPx(dims(1)), posPx(dims(0))));
        posPx = view->um2Px(posUm);
        path.lineTo(QPointF(posPx(dims(1)), posPx(dims(0))));
        painter->drawPath(path);
...
void IRoCSSCTChannelSpecsOrthoViewRenderer::render(
    ...
    iRoCS::ProgressReporter *pr) const
{
  ...
  blitz::TinyVector<float,3> posPx(
      (channel->p_sct->getAxisPosition(start) - lowerBoundUm) * um2PxFactor);

  QString pathSpec = "M" + QString::number(posPx(dimensions(1))) + " " +
      QString::number(posPx(dimensions(0)));
  for (double u = start + step; u <= end; u += step)
  {
    posPx = (channel->p_sct->getAxisPosition(u) - lowerBoundUm) * um2PxFactor;
    ...
    if (lambda(d) > 0)
    {
      ...
      if (blitz::all(posUm >= lowerBoundUm && posUm <= upperBoundUm))
      {
        ...
        QString pathSpec =
            ...
            QString::number(posPx(dimensions(0)));
        posPx = (channel->p_sct->getAxisPosition(start) - lowerBoundUm) *
            um2PxFactor;
        pathSpec = pathSpec + " L" + QString::number(posPx(dimensions(1))) +
            " " + QString::number(posPx(dimensions(0)));
        svgStream.writeEmptyElement("path");
  ...
  for (int d = 0; d < 6; ++d)
  {
    if (lambda(d) > 0)
    {
      ...
      if (blitz::all(posUm >= lowerBoundUm && posUm <= upperBoundUm))
      {
        posPx = (channel->p_sct->getAxisPosition(end) - lowerBoundUm) *
            um2PxFactor;
        QString pathSpec =
            "M " + QString::number(posPx(dimensions(1))) + " " +
            QString::number(posPx(dimensions(0)));
