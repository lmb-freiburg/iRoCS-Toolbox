#include "TrainDetectorWorker.hh"
...
TrainDetectorWorker::TrainDetectorWorker(
...
#pragma omp parallel for
         ...
         it != p_annotation->end(); ++it)
    {
      if ((*it)->label() != -1)
      {
        trainingSet.push_back(
            svt::BasicFV((*it)->features(),
                         ((*it)->label() > 0) ? 1.0f : -1.0f, trainIndex));
        if ((*it)->label() > 0) nPositives++;
        else nNegatives++;
        trainIndex++;
      }
    }
  ...
  if (nPositives == 0 || nNegatives == 0)
  {
    if (p_progress != NULL)
        p_progress->abortWithError(
            tr("The given annotations only contain samples of one class. "
               "Discriminative training impossible!").toStdString());
    return;
  }
...
// #define TRAINDETECTORDEBUG
#ifdef TRAINDETECTORDEBUG
  {
