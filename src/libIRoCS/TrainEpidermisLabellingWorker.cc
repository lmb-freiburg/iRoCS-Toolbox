      ...
      TrainingParameters const &parameters, ProgressReporter *pr)
  {
    ...
    for (size_t fileIdx = 0; fileIdx < trainFiles.size(); ++fileIdx)
    {
      ...
      for (size_t i = 0; i < nuclei.size(); ++i)
          nuclei[i].features().resize(nFeatures);
...
#pragma omp parallel for
      ...
      for (size_t k = 0; k < nuclei.size(); ++k)
      {
        if (nuclei[k].label() != -1)
        {
          int label = (nuclei[k].label() != 0 && nuclei[k].label() != 2) ?
              -1 : nuclei[k].label();
          if (nuclei[k].mitotic()) label = 14;
    ...
    if (nEpidermis == 0)
    {
      if (pr != NULL)
          pr->abortWithError(
              "The given annotation does not contain epidermis interphase "
              "samples. Discriminative training impossible!");
      return;    
    }
    if ((nOtherNucleus + nNoNucleus + nMitoses) == 0)
    {
      if (pr != NULL)
          pr->abortWithError(
              "The given annotation only contains Epidermis samples. "
