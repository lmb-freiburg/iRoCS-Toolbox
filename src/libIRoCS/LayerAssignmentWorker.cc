#include "LayerAssignmentWorker.hh"
...
namespace iRoCS
{
  ...
  void assignLayers(
      ...
      std::string const &cacheFileName, bool updateMitoses,
      bool cacheCoordinates, bool forceFeatureComputation,
      ProgressReporter *pr)
  {
    double sigmaMin = 0.5;
    ...
    for (size_t i = 0; i < testVectors.size(); ++i)
        testVectors[i].resize(nFeatures + 3);
    ...
    double progressStepPerFeatureLoad =
        progressLoadFeatures / static_cast<double>(nFeatures + 3);
...
#pragma omp parallel for
    ...
    // Compute voxelwise intrinsic coordinate features if requested
    if (cacheCoordinates && cacheFileName != "")
        features.intrinsicCoordinates(data, iRoCS, cacheFileName);

    // Compute intrinsic coordinate features for markers
#ifdef _OPENMP
#pragma omp parallel for
#endif
    for (ptrdiff_t j = 0; j < static_cast<ptrdiff_t>(nuclei.size()); ++j)
    {
      ...
      blitz::TinyVector<double,3> p(
          iRoCS.getCoordinates(nuclei[j].positionUm()));
      testVectors[j][nFeatures] = p(2);
      testVectors[j][nFeatures + 1] = p(0);
      testVectors[j][nFeatures + 2] = p(1);
      nuclei[j].setQcDistanceUm(p(0));
      nuclei[j].setRadialDistanceUm(p(1));
      nuclei[j].setPhi(p(2));
    }
    
    progress += 3.0 * progressStepPerFeatureLoad;
    if (pr != NULL)
    {
      if (!pr->updateProgress(static_cast<int>(progress))) return;
      pr->updateProgressMessage("Normalizing features");
    }
...
#pragma omp parallel for
    ...
    for (ptrdiff_t j = 0; j < static_cast<ptrdiff_t>(nuclei.size()); ++j)
    {
      ...
      nuclei[j].setPredictedLabel(label);
      if (updateMitoses) nuclei[j].setPredictedMitotic(mitotic);
      if (!nuclei[j].manual())
      {
        nuclei[j].setLabel(label);
        if (updateMitoses) nuclei[j].setMitotic(mitotic);
      }
    }
  }
