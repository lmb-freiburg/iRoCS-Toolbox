...
void roots3(double d[3], double c0,double c1, double c2)
{
  ...
  angle = std::atan2(std::sqrt(-q), mbDiv2) * inv3;
  cs = std::cos(angle);
  sn = std::sin(angle);
...
int fast_eigen3x3(double A[3][3], double V[3][3], double d[3], int sortMode)
{
  ...
  double da[3];
  check = (absd(A[0][1]) < smallv) + (absd(A[0][2]) < smallv) +
      (absd(A[1][2]) < smallv);
  if (check > 1) return 0;

  /* 0 = - det (A - yI) = y^3 + c2 * y^2 + c1 *y + c0 */
  c0 = -(A[0][0] * A[1][1] * A[2][2] + 2 * A[0][1] * A[0][2] * A[1][2] -
         A[0][0] * pow2(A[1][2]) - A[1][1] * pow2(A[0][2]) -
         A[2][2] * pow2(A[0][1]));
  c1 = A[0][0] * A[1][1] - pow2(A[0][1]) + A[0][0] * A[2][2] -
      pow2(A[0][2]) + A[1][1] * A[2][2] - pow2(A[1][2]);
  ...
  l1 = std::sqrt(pow2(V[0][0]) + pow2(V[1][0]) + pow2(V[2][0]));
  l2 = std::sqrt(pow2(V[0][1]) + pow2(V[1][1]) + pow2(V[2][1]));
  l3 = std::sqrt(pow2(V[0][2]) + pow2(V[1][2]) + pow2(V[2][2]));
  
  /* Detect fail : eigenvectors with only zeros */
  if (l1 < smallv || l2 < smallv || l3 < smallv) return 0;
    ...
    double A[][3], double V[][3], double d[], int sortMode)
{
  ...
  if (fast_eigen3x3(A, V, d, sortMode)) return;

  for (i = 0; i < n; i++) for (j = 0; j < n; j++) V[i][j] = A[i][j];
  tred2(V, d, e);
  tql2(V, d, e);
