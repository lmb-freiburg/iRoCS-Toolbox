...
#ifndef ATBFASTPHASEONLYCORRELATIONFILTER_HH
#define ATBFASTPHASEONLYCORRELATIONFILTER_HH
...
/*======================================================================*/
/*!
 *  \class FastPhaseOnlyCorrelationFilter "libArrayToolbox/FastPhaseOnlyCorrelationFilter.hh"
 *  \brief The FastPhaseOnlyCorrelationFilter class provides phase-only
 *    cross-correlation for n-D Arrays employing the Fast Fourier Transform.
 *
 *  Specifically it uses the BlitzFFTW library. This is the fastest
 *  possibility to compute the phase-only cross-correlation of two data
 *  Arrays. It uses lots of memory, though, because it has to hold padded
 *  versions of the data Array, the kernel and the corresponding fourier
 *  transforms, therefore a memory overhead of factor 4 is the minimum,
 *  factor 6 to 8 is common.
 */
/*======================================================================*/
  template<typename DataT, int Dim>
  class FastPhaseOnlyCorrelationFilter : public Filter<DataT,Dim,DataT>
  {
 ...
 *   Constructor.
 *
 *   \param kernel         The filter kernel. An immutable refererence of
 *     the kernel will be stored, and will be transformed to Fourier domain
 *     on demand. Once transformed changes to the kernel Array will not
 *     be respected. Then you have to create a new
 *     FastPhaseOnlyCorrelationFilter.
 *   \param bt             The boundary treatment this filter uses. CropBT
 *     is not allowed for the FastPhaseOnlyCorrelationFilter and will
...
/*======================================================================*/
  ...
  private:
    ...
    blitz::Array<DataT,Dim> const &_kernel;
    mutable blitz::Array<std::complex<DataT>,Dim> _kernelFFTCache;
    static DataT const eps;

  };

}

#include "FastPhaseOnlyCorrelationFilter.icc"
