...
void NucleusMarker::save(const AnnotationChannelSpecs* channel,
                         BlitzH5File& outFile, const std::string& group)
{
  if (channel->markers().size() == 0) return;
  SHSurfaceMarker::save(channel, outFile, group);
...
void NucleusMarker::load(AnnotationChannelSpecs* channel,
                         const BlitzH5File& inFile, const std::string& group)
{
  SHSurfaceMarker::load(channel, inFile, group);
